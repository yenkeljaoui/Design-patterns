...
public class Hybrid_Plane extends Air implements IsLand, IsMarine, Motorized, Vehicles_Interface{
    private Land land;
    private Marine marine;
    private int fuelConsumption;
    private int engineLife;
    public Hybrid_Plane(String model, int maxSpeed, int numPass, int numWheels, boolean sailingDirection,
                        String flag, int fuelConsumption, int engineLife, ImageIcon im) {
        /**
         * Constructor of the Hybrid plane class inheriting from Land and Marine classes
         * I used a delegation.
         */
        super(model, maxSpeed, numPass, "military");
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof Hybrid_Plane))
            return false;
        Hybrid_Plane hybridPlane = (Hybrid_Plane) obj;
        if (super.equals(obj))
        {
            if (getFlag().equals(hybridPlane.getFlag()) && getWayType().equals(hybridPlane.getWayType())
                    && getUse().equals(hybridPlane.getUse()))
                return  hybridPlane.getSailingDirection() == getSailingDirection()
                        && hybridPlane.getNumWheels() == getNumWheels() && hybridPlane.getFuelConsumption() == getFuelConsumption()
                        && hybridPlane.getEngineLife() == getEngineLife();
        }
        return false;
    }
}
